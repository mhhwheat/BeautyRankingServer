...
public class ConnectionPool {
    ...
    private String dbUrl = "jdbc:mysql://192.168.202.197:3306/beauty_ranking?characterEncoding=utf8"; // 数据 URL
    private String dbUsername = "myuser"; // 数据库用户名
    private String dbPassword = "mypassword"; // 数据库用户密码
...
// 它中存放的对象为 PooledConnection 型
    ...
    public ConnectionPool(){
    	try {
			createPool();//饿汉式单例模式，空间换时间，不可以this.connPool.createPool()因为connpool还没创建
		} catch (Exception e) {
            ...
            if (!pConn.isBusy()) {
                ...
                if (!testConnection(conn)) {
                    ...
                    try {
                        ...
                        System.out.println(" 失效的数据库连接再次创建失败！ " + e.getMessage());
                        return null;
                    }
